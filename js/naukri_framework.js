var naukriFrameWork=function(id){
	var me=this;
...
//	console.log(el.constructor.toString().indexOf('Array'), el.length)
	...
	me.addEvent('submit',function(e){if(state==false){
			return (e&&e.preventDefault)?e.preventDefault():false;			
			}
		else return true;
		 })//Add Event
	return me;
	...
	me.show = function () {
		for(var i=0; i<me.length; i++){
			...
			el[i].style.display = (getStyle!='inline') ? 'block' : 'inline';
		}
		return me;
	}
	me.hide=function () {
	...
	me.toggle=function(){
		for (var i=0; i<me.length; i++){
			(me.getStyle(el[i], 'display') != 'none') ? me.hide() : me.show();
		}
		return me;
	}
	...
	me.childrens=function(type){
		...
		for (var i=0; i<me.length; i++){
				var a=el[i].getElementsByTagName(type || '*');
...
//			alert(me.getObj(child))
	...
	me.prepend = function(child){
		for (var i=0; i<me.length; i++){
			el[i].insertBefore(me.getObj(child), naukriFrameWork(el[i]).first());
		}
		return me;
	}
	me.getObj=function(child){
		if((child.toString()).toLowerCase()=='[object object]')
			child=child.currObj();
...
/*	me.attr = function(){
		var bool=(typeof(arguments[0])).toLowerCase()!='string';
		var isIE7 = false;
		/*@cc_on
	...
	/*	for (var i=0; i<me.length; i++){
			if(bool){
				for(var x in arguments[0]){
					var key=x;
					for(var y in me.ieFix){
						if(y==x.toLowerCase() && isIE7){
							key=me.ieFix[y];
							break;
						}
					}
					if(arguments[0][x])
					el[i].setAttribute(key, arguments[0][x])
					else
					el[i].setAttribute(key, '')
				}
			}
			else{
				var key=arguments[0];
				for(var y in me.ieFix){
					if(y==arguments[0] && isIE7){
							key=me.ieFix[y];
							break;
					}
				}
				return el[i].getAttribute(key);
			}
		}
		return me;
	}*/
...
chainingFunction.prototype.ajaxReq = function(param){
	...
	this.createXHR();
	xhr.open(type.toUpperCase(),url,async.toLowerCase());
